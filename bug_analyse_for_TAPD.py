...
class bug_analyse():
    ...
    def __init__(self):
        self.bug_info = {'created_bug_count': 0, 'resolved_bug_count': 0, 'online_bug_count': 0, 'offline_bug_count': 0,
                         'severity_count': {}, 'resolution_method_count': {}}  #用于bug维度存储数据
        self.bug_on_person = {}  #用户存储个人维度的bug数据，内部结构{'name':{'resolve_bug':10, 'report_bug':0, 'response_bug':0, 'refuse_bug':0, 'resolve_time':[1,2,3]}, }
    ...
    def bug_online_analyse(self, buglist=[]):
        ...
        print("BUG线上线下分类，其中线上{}个，占比{}；线下{}个，占比{}".format(self.bug_info['online_bug_count'], "{:.1f}%".format(
            self.bug_info['online_bug_count'] / self.bug_info['created_bug_count'] * 100), self.bug_info['offline_bug_count'],
                                                                           "{:.1f}%".format(self.bug_info['offline_bug_count'] / self.bug_info['created_bug_count'] * 100)))
        print("************************************************************")
        ...
        for res in self.bug_on_person:
            if 'report_bug' in self.bug_on_person[res]:
                print("{}报告{}个，占比{}；".format(res, self.bug_on_person[res]['report_bug'],
                                                                 "{:.1f}%".format(
                    self.bug_on_person[res]['report_bug'] / self.bug_info['created_bug_count'] * 100)))
        print("************************************************************")
    ...
    def total_analyse(self):
        ...
        print("{}至{}区间内共新建{}个bug，修复{}个bug".format(startdate.strftime('%Y-%m-%d'), enddate.strftime('%Y-%m-%d'),
                                                 self.bug_info['created_bug_count'], self.bug_info['resolved_bug_count']))
